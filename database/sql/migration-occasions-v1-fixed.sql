-- =====================================================
-- FloresYa: Migración para Ocasiones v1.0 - CORREGIDA
...
JOIN occasions o ON (
    ...
    (p.occasion = 'valentine' AND o.name = 'San Valentín') OR
    ...
    (p.occasion = 'funeral' AND o.name = 'Condolencias') OR
...
-- Paso 8: Vista para productos con ocasiones (CORREGIDA)
...
-- Paso 9: Función auxiliar para obtener productos con ocasiones
...
UNION ALL
SELECT 
    'products_migrated' as table_name,
...
SELECT 
    p.id,
    p.name,
    p.occasion as old_occasion,
    STRING_AGG(o.name, ', ' ORDER BY o.sort_order) as new_occasions
FROM products p
LEFT JOIN product_occasions po ON p.id = po.product_id
LEFT JOIN occasions o ON po.occasion_id = o.id
WHERE p.occasion IS NOT NULL
GROUP BY p.id, p.name, p.occasion
ORDER BY p.id;