...
async function verifyNewSystemOnly() {
    log(`${colors.bold}🔍 VERIFICANDO SISTEMA NUEVO EXCLUSIVAMENTE${colors.reset}`, 'blue');
    ...
    try {
        ...
        if (statsError) {
            log(`❌ Error accediendo a tabla product_images: ${statsError.message}`, 'red');
        ...
        if (totalImages === 0) {
            log(`⚠️  Tabla product_images vacía - ejecuta migración primero`, 'yellow');
            return false;
        }
        
        // 2. Verificar productos con imágenes
        const { data: productsWithImages } = await supabase
            .rpc('get_products_with_image_count');
        ...
        for (const product of products || []) {
            ...
            if (imageCount > 0) {
                ...
                if (primaryImage) {
                    log(`      🖼️  Principal: ${primaryImage.url_large.split('/').pop()}`, 'blue');
                }
            } else {
        ...
        try {
            ...
            if (data.success && data.data.products.length > 0) {
                ...
                if (hasLegacyFields) {
                    log(`   ⚠️  API aún devuelve campos legacy (image_url/additional_images)`, 'yellow');
                ...
                if (product.images && Array.isArray(product.images)) {
                    log(`   ✅ API devuelve campo 'images' con ${product.images.length} imágenes`, 'green');
                } else {
        ...
        const systemHealth = {
            ...
            hashIntegrity: `${hashIntegrityPassed}/${sampleImages?.length || 0} válidos`
        };
        
        log(`📊 Imágenes totales: ${systemHealth.totalImages}`, 'blue');
        log(`✅ Productos con imágenes: ${systemHealth.productsWithImages}`, 'green');
        log(`❌ Productos sin imágenes: ${systemHealth.productsWithoutImages}`, productsWithoutImages > 0 ? 'red' : 'green');
        log(`🔐 Integridad SHA256: ${systemHealth.hashIntegrity}`, 'blue');
        
        if (totalImages > 0 && productsWithImageData > 0 && productsWithoutImages === 0) {
            log(`\n🎉 ¡SISTEMA NUEVO 100% OPERATIVO!`, 'green');
            log(`✅ Todos los productos tienen imágenes en product_images table`, 'green');
            log(`✅ Sistema legacy completamente reemplazado`, 'green');
            return true;
        } else {
            log(`\n⚠️  Sistema en transición`, 'yellow');
            if (productsWithoutImages > 0) {
                log(`💡 ${productsWithoutImages} productos necesitan migración`, 'yellow');
            }
            return false;
        }
...
async function forceCompleteSystem() {
    ...
    try {
        ...
        const productsNeedingImages = productsWithoutImages.filter(p => 
        ...
        log(`🎉 ${productsNeedingImages.length} productos ahora tienen imágenes`, 'green');
...
async function main() {
    ...
    if (!isComplete) {
        log(`\n❓ ¿Forzar sistema completo con placeholders? (requiere confirmación manual)`, 'yellow');
