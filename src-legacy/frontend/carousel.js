/**
 * 🌸 FloresYa Professional Carousel Service - JavaScript
 * Enhanced carousel with industry-standard features (ES6+ compatible)
 * Solves rendering, sizing, and interaction issues
...
class CarouselService {
  constructor(config = {}, events = {}) {
    ...
    // Event callbacks
    this.events = {
      onSlideChange: events.onSlideChange || null,
      onSlideChanged: events.onSlideChanged || null,
      onError: events.onError || null,
      onLoad: events.onLoad || null,
      onImageLoad: events.onImageLoad || null,
      onImageError: events.onImageError || null,
    };
  ...
  normalizeImages(apiImages) {
    return apiImages
      ...
      .map((img, index) => {
        ...
        return {
          id: (img.id ? img.id.toString() : `carousel-${index}`),
          url: optimizedUrl,
          ...
          is_active: img.is_active !== undefined ? img.is_active : (index === 0)
        };
      })
      ...
      return new Promise((resolve) => {
        ...
        img.onerror = (error) => {
          ...
          if (this.events.onImageError) {
            this.events.onImageError(index, image, error);
          }
          resolve();
        };
  ...
  setupEventListeners() {
    ...
    this.carouselElement.addEventListener('keydown', (event) => {
    ...
    this.carouselElement.addEventListener('touchstart', (event) => {
      startX = event.touches[0].clientX;
    });

    this.carouselElement.addEventListener('touchend', (event) => {
  ...
  pause() {
    if (this.carouselInstance && this.carouselInstance.pause) {
  ...
  log(message, level = 'info') {
    ...
    console.log(`%c[🌸 Carousel JS] ${message}`, style);
  }
